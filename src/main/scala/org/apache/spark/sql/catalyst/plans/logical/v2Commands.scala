 ...
 */
case class ShowCurrentNamespace(catalogManager: Object) extends Command {
  override val output: Seq[Attribute] = Seq(
...
case class LoadData(
    ...
    isLocal: Boolean,
    isOverwrite: Boolean,
    partition: Option[Object]) extends Command {
  override def children: Seq[LogicalPlan] = child :: Nil
}
...
case class TruncateTable(
    child: LogicalPlan,
    partitionSpec: Option[Object]) extends Command {
  override def children: Seq[LogicalPlan] = child :: Nil
}
