...
import org.apache.spark.sql.connector.expressions.{BucketTransform, DaysTransform, Expression => V2Expression, FieldReference, HoursTransform, IdentityTransform, LiteralValue, MonthsTransform, Transform, YearsTransform}
...
class AstBuilder extends SqlBaseBaseVisitor[AnyRef] with SQLConfHelper with Logging {
   ...
   */
  type InsertDirParams = (Boolean, Object, Option[String])
  ...
  private def withInsertInto(
    ...
    ctx match {
      ...
      case dir: InsertOverwriteDirContext =>
        ...
        throw new UnsupportedOperationException("This functionality removed for microbase.")
   ...
   * Create a [[BucketSpec]].
   */
  override def visitBucketSpec(ctx: BucketSpecContext) = withOrigin(ctx) {
    ???
    // BucketSpec(
   ...
   */
  type TableClauses = (
      Seq[Transform], Seq[StructField], Option[Object], Map[String, String],
  ...
  private def cleanNamespaceProperties(
    ...
    val legacyOn = false; //conf.getConf(SQLConf.LEGACY_PROPERTY_NON_RESERVED)
    properties.filter {
